...
function list_all_task(){
    for(let i=0; i<task_list.length; i++){
        if(task_list[i] != null){
            append_one_task_in_group(task_list[i].task_name,task_list[i].task_discription,task_list[i].task_reward,task_list[i].task_quota)
        }
    }
}

function append_one_task_in_group(add_task_name,add_task_discription,add_task_reward,add_task_quota){
    let task_group = document.getElementById("task-group");
    task_group.insertAdjacentHTML('beforeend','\
    <div class="flex justify-center items-Stretch rounded-xl border-2 p-3  pr-0 pb-0 sm:pr-0 sm:pb-0 mb-2 w-full border-slate-300 bg-amber-200 hover:border-slate-400 overflow-hidden">\
        <span class="text-2xl text-amber-950 flex-1 pb-2 font-bold" >\
            ...
            <span>剩餘數量:</span>\
            <span class="task-quota">'+add_task_quota+'</span>\
            </div>\
        </span>\
        <span class="flex-none flex items-end" >\
            <button class="achieve-but text-xl sm:text-2xl sm: hover:bg-yellow-500 bg-amber-300 text-yellow-700  hover:text-white py-2 px-2 sm:px-1 hover:border-transparent">\
                達成\
            </button> \
        </span>\
    </div>\
...
function open_create_task_windows(){
    ...
    <div class="pop_up_win w-full h-screen fixed top-0 bg-stone-950/50 flex justify-center items-center"> \
        <div class="add-task-win w-[96%] bg-stone-200 rounded-xl relative"> \
            <div class="p-8"> \
                <div class="text-3xl text-center"> 發布任務 </div> \
                <div class="mt-5 text-xl"> 任務名稱 </div> \
                <div class="flex justify-center"> \
                <input id="add-task-name" type="text" class="w-full text-xl p-3"> \
                </div> \
                <div class="mt-5 text-xl"> 任務詳情 </div> \
                <div class="flex justify-center"> \
                    <textarea id="add-task-discription" rows="3" cols="70" class="w-full text-xl p-3"></textarea> \
                </div> \
                <div class="mt-5 text-2xl flex justify-end"> \
                    <input id="add_task_reward" type="number" value="10" class="mx-1 text-center" style="width:3em;"> \
                    <span class="mx-1">枚金幤</span> \
                    <span class="mx-1">酬勞</span> \
                </div> \
                <div class="mt-5 text-2xl flex justify-end">\
                    <span class="mx-1">任務數量</span>\
                    <input id="add_task_quota" type="number" value="1" class="mx-1 text-center" style="width:3em;">\
                </div>\
                <div class="mt-12 flex justify-center"> \
                <button id="submit-task-bnt" type="button" class="focus:outline-none w-5/6 text-white bg-yellow-500 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-lg px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900">提交</button> \
                </div> \
            <div class="close-bnt absolute top-1 right-4 text-3xl select-none"> x </div> \
        </div> \
    </div>');
    ...
    submit_task_bnt.addEventListener("click",()=>{
        ...
        add_task_quota = document.getElementById("add_task_quota").value;
        ...
        new_task.task_quota = parseInt(add_task_quota);
...
function list_all_reward(){
    for(let i=0; i<reward_list.length; i++){
        if(reward_list[i] != null){
            append_one_reward_in_group(reward_list[i].reward_name,reward_list[i].required_coins,reward_list[i].reward_quota);
        }
    }
    reset_getreward_but_ainmation();
}

function append_one_reward_in_group(reward_name,required_coins,reward_quota){
    ...
    <button class="get-reward-but text-xl sm:text-2xl min-w-[5rem] hover:bg-yellow-500 bg-amber-300 text-yellow-700  hover:text-white py-2 px-1 hover:border-transparent flex-none">\
    換領\
    </button> \
...
function open_create_reward_windows(){
    ...
    <div class="pop_up_win w-full h-screen fixed top-0 bg-stone-950/50 flex justify-center items-center">\
        <div class="add-task-win w-[96%] bg-stone-200 rounded-xl relative">\
            <div class="p-8">\
                <div class="text-3xl text-center"> 添加獎勵 </div>\
                <div class="mt-5 text-xl"> 獎勵名稱 </div>\
        <div class="flex justify-center"> \
            <input id="add-reward-name" type="text" class="w-full text-xl p-3"> \
        </div>\
        <div class="mt-5 text-2xl flex justify-end">\
            <span class="mx-1">所需</span>\
            <input id="required_coins" type="number" value="10" class="mx-1 text-center" style="width:3em;"> \
            <span class="mx-1">金幤</span>\
        </div>\
        <div class="mt-12 flex justify-center">\
